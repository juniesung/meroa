<!--
================================================================================
DRAFT — LEGALLY BINDING — REQUIRES YOUR REVIEW BEFORE PUBLISHING
This is not legal advice. It is engineering-drafted copy grounded in the
code-verified data inventory (docs/data-inventory.md). Have it reviewed before
it is served publicly (Phase 8, Item 5 renders this to HTML from the Hono
server). Fill every [PLACEHOLDER] before publishing.
================================================================================
-->

# Privacy Policy

**Effective date:** July 20, 2026
**Last updated:** July 20, 2026

This Privacy Policy explains what information Meroa ("Meroa", "we", "us") collects,
how we use it, and the choices you have. Meroa is a relationship-first AI companion
app. If you do not agree with this policy, please do not use Meroa.

**Who we are.** Meroa is operated by Jun Kwon,
California. You can reach us at meroa.app@gmail.com.

---

## The short version

- You sign in with **Sign in with Apple**; we collect your **Apple account identifier**
  and, if you choose to share it, your **name**. We collect the **messages** you send Meroa
  and the **tasks, goals, and memories** you create in the app. We **do not** collect your
  phone number.
- Your **messages are sent to a third-party AI service** to generate replies. We ask for
  your explicit permission before this happens, and you can withdraw it at any time.
- We **do not** use advertising, we **do not** sell your data, and we **do not** track you
  across other apps or websites.
- You can **export** your data and **delete your account** — a deletion permanently and
  immediately removes your data from our systems.

---

## 1. Information we collect

We only collect information you give us by using the app. We collect:

- **Sign-in information.** You sign in with **Sign in with Apple**. We receive an **Apple
  account identifier** that identifies your account and, if you choose to share it, your
  **name** (used to greet you). We do **not** collect your phone number.
- **Your messages.** The messages you send Meroa and the replies Meroa generates.
- **Your content.** Tasks, goals, progress entries, and records you create.
- **Memories.** Facts Meroa remembers to personalize your experience (for example, a
  preference or a goal you mentioned). You can view, edit, mark sensitive, suppress, or
  delete these at any time.
- **Preferences.** Settings such as your chosen chat style, quiet hours, timezone, and your
  AI data-sharing consent.
- **Subscription status.** Whether you have an active subscription (verified through the
  app store and our payments processor).
- **Diagnostics.** Technical error reports, used to find and fix crashes and bugs.
- **Notification token.** If you turn on notifications, a device push token so we can
  deliver reminders and check-ins. It is used only to send you notifications, and you can
  turn notifications off at any time in your device settings.
<!-- REVIEW (2026-07-26, RESOLVED 2026-07-30): push is confirmed in v1.0 scope (token
     registration verified on device), so this notification-token bullet and the push-provider
     row in §3 stand. -->
<!-- NOTE (2026-07-30): auth reconciled to Sign in with Apple — §"short version", §1, §3, §4,
     §6 updated (name + Apple identifier in, phone number out; login-codes references removed). -->


**What we do not collect.** We do **not** collect location, contacts, photos, your
microphone or camera, device advertising identifiers, or any product-analytics tracking.
We do not use third-party analytics or advertising SDKs.

## 2. How we use your information

- To provide the core service: hold a conversation, create and update your tasks and goals,
  and remember relevant context over time.
- To sign you in and keep you signed in securely.
- To generate AI replies (see Section 3).
- To manage your subscription and verify entitlement.
- To keep the app reliable (diagnosing errors and crashes).

We do **not** use your information for advertising, and we do **not** sell it.

## 3. AI processing and third-party sharing

Meroa's replies are generated by a **third-party AI service**. When you send a message,
the **content of that message** — together with information about your tasks and goals
needed to respond — is transmitted over an encrypted connection (HTTPS) to that service to
produce a reply. We do **not** send your name or any account identifier to the AI
service.

**Your consent is required.** Before any of your messages are sent to the AI service, we
ask for your explicit permission inside the app. **You can withdraw this permission at any
time** in Settings; if you do, chat will stop working until you grant it again, because the
feature cannot function without it.

We share information with a small number of service providers, only as needed to run Meroa:

| Provider | What it receives | Why |
|---|---|---|
| Our AI provider (a third-party AI service) | The content of your messages and related task/goal context | To generate replies |
| Our payments provider and the app stores (Apple / Google) | Your subscription status and an account identifier | To manage subscriptions |
| Our error-monitoring provider | Technical error reports | To keep the app working |
| Our push-notification provider | Your device notification token and the text of the notification to deliver | To send you reminders and check-ins |
| Our hosting provider | Hosts our servers and database | Infrastructure |

These providers process data **on our behalf** under our instructions; they are not
permitted to use it for their own purposes.
<!-- REVIEW (2026-07-26, RESOLVED): the "on our behalf / not for their own purposes" claim is
     now TRUE for the AI provider. Production uses OpenAI (gpt-5-mini + gpt-5-nano), which does
     not train on API data by default and processes in the US. DeepSeek — whose API terms
     allowed training + PRC retention — was replaced precisely so this line holds. Re-verify
     only if the AI provider changes. -->

We do not otherwise share, rent, or sell your personal information.

## 4. How your information is protected

Information is **encrypted in transit** (HTTPS) and **encrypted at rest** in our database.
Session tokens are stored **hashed**, not in plain text.

## 5. How long we keep it

We keep your information for as long as your account exists. **When you delete your
account, we permanently and immediately delete your data** from our systems (see Section 6).

Deleting your account removes your data from Meroa; it does **not** cancel a subscription
purchased through the App Store or Google Play. To stop billing, cancel the subscription in
the App Store or Google Play.

## 6. Your rights and choices

- **Access / export.** You can export a copy of your data from within the app.
- **Correction / deletion of memories.** You can edit, suppress, or delete individual
  memories, and tell Meroa not to bring something up.
- **Account deletion.** You can delete your account and all associated data from within the
  app, or via our web deletion page at https://meroa-production.up.railway.app/account/delete. Deletion is immediate and
  permanent.
- **Withdraw AI consent.** You can withdraw permission for AI processing at any time in
  Settings.

### California privacy rights (CCPA/CPRA)

If you are a California resident, you have the following rights over your personal
information:

- **Right to know / access** — request the categories and specific pieces of personal
  information we have collected about you, the sources we collected it from, the business
  purpose for collecting it, and the categories of third parties we share it with.
- **Right to delete** — request deletion of the personal information we hold about you. You
  can do this yourself at any time by deleting your account (above); deletion is immediate
  and permanent.
- **Right to correct** — request that we correct inaccurate personal information about you.
- **Right to opt out of sale or sharing** — we do **not** sell your personal information and
  do **not** share it for cross-context behavioral advertising, so there is nothing to opt
  out of.
- **Right to limit the use of sensitive personal information** — your messages may contain
  sensitive information. We use it only to provide the service to you (to reply, and to
  create and update the tasks, goals, and memories you ask for); we do not use it to infer
  characteristics about you, and we do not sell or share it.
- **Right to non-discrimination** — we will not discriminate against you for exercising any
  of these rights.

To exercise these rights, email us at meroa.app@gmail.com or use the in-app controls (data
export, memory management, and account deletion). We verify requests using your account
(your Sign in with Apple identity), and you may use an authorized agent to submit a request
on your behalf. If you live outside California, you may have other rights under your local law
— contact us and we will do our best to help.

<!-- REVIEW: CCPA/CPRA section drafted from what the app actually does; still requires counsel review before publishing (not legal advice). -->


## 7. Children

Meroa is not directed to children under 13, and we do not knowingly collect information
from them.

## 8. International users and data transfers

Meroa is operated from the United States, and your information is stored and processed in
the **United States** (our servers and database are hosted in a US-West region, and the
third-party AI service that generates replies also processes your messages in the United
States).

If you access Meroa from outside the United States, you understand that your information
will be transferred to, stored, and processed in the United States, where data-protection
laws may differ from those in your country. Where a transfer of personal information is
subject to laws requiring a specific safeguard, we will take steps to provide an appropriate
one.

<!-- REVIEW: international/data-transfer clause drafted; still requires counsel review (not legal advice). AI provider is now OpenAI (US-processed), so the earlier "message content processed outside the US / PRC" concern no longer applies. -->


## 9. Changes to this policy

We may update this policy. If we make a material change to how your data is shared with the
AI service, we will ask for your consent again before continuing.

## 10. Contact

Questions about this policy or your data: meroa.app@gmail.com.
