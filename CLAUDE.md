...
| 5   | **The connected loop** (one record ↔ Chat/Tasks/Goals, undo, confirmation gating) — one stored record renders everywhere, all four goal types, task↔goal linking, undo, and history-aware replies; DoD protocol run passed (found + fixed 3 live bugs). Next gate: the **provider decision** (deepseek-v4-flash is still only the test provider) | `docs/phases/phase-5-connected-loop.md`               | ☑      |
| 6   | Personalization, memory & trust (vibe, adaptive style, memory controls, quiet hours) — **read `docs/chat-architecture.md` first.** Presets change *how* the reply pass talks; memories are stored data **injected into context and quoted**, never model recall — so they follow the same rule as every number in the app | `docs/phases/phase-6-personalization-memory-trust.md` | ☐      |
