...
export const AI_TOOLS: Anthropic.Tool[] = [
    ...
    input_schema: {
      ...
      properties: {
        ...
        stages: {
          ...
          items: { type: 'string' },
          description:
            'milestone only: 2-8 ordered stage titles, e.g. ["Applying", "Interviewing", "Offer negotiation"] for "land a summer internship" — ONLY if the user actually named their milestones. These are the USER\'S milestones, never yours — use the ones they actually named, in their order, however obvious a sequence looks for this kind of goal. If they haven\'t named any, omit this field entirely (do not call no_action, do not ask) — create_goal still goes up, as a bare name-only template the user fills in via the Goals tab. Never for any other goal type.',
        },
        starterTasks: {
          type: 'array',
          items: {
  ...
  },
  {
    ...
    description:
      'MILESTONE goals only: request moving a milestone goal to its next stage — this does NOT move it immediately. It shows the user a card that lists the current stage\'s open tasks getting retired and the next stage\'s tasks, so they can confirm or cancel themselves; nothing changes until they tap Advance. Completing a linked task is NEVER by itself a reason to call this — a task finishing is not the same as the user declaring the stage done; only call this on the user\'s own clear statement, and call it RIGHT AWAY once they\'ve made it — never ask "should I move you to the next stage?" (the tap on the card is that confirmation) and never wait to gather the next stage\'s tasks first. The next stage\'s tasks were most likely already planned ahead of time in the Goals tab and will show up on the card automatically; only pass nextStageTasks if the user states new ones for it in the very same breath as declaring the stage done ("got the interview! now I need to do mock interviews and study leetcode") — never invent them, and never call no_action to go ask for them. If this is the LAST stage there is no next stage, and it proposes completing the goal (no new tasks).\n\nUse the goal\'s ref from the goals list in context, never a guess.',
    input_schema: {
      type: 'object',
      properties: {
        goalRef: GOAL_REF_PROPERTY,
        ...
        nextStageTasks: {
          ...
          description:
            'Optional — only if the user stated new to-dos (up to 5) for the next stage in the same message as declaring the current one done, exactly as they gave them, never a contribution amount (a milestone goal never logs a number from a task). Omit this in every other case: the next stage\'s tasks, if any were planned ahead of time in the Goals tab, are added automatically when the card is confirmed. Never invent tasks here.',
        },
      },
      required: ['goalRef', 'nameHint'],
    },
