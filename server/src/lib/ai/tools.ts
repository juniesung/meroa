...
export const AI_TOOLS: Anthropic.Tool[] = [
    ...
    input_schema: {
      ...
      properties: {
        ...
        stages: {
          ...
          items: { type: 'string' },
          description:
            'milestone only, required there: 2-8 ordered stage titles, e.g. ["Applying", "Interviewing", "Offer negotiation"] for "land a summer internship". These are the USER\'S milestones, not yours — use the ones they actually named, in their order. If they haven\'t said what the milestones are, do NOT call this tool: call no_action instead and the reply pass will ask them. Never invent a sequence, however obvious it looks for this kind of goal. Never for any other goal type.',
        },
        starterTasks: {
          type: 'array',
          items: {
          ...
          description:
            'Proposed starter tasks (up to 5). For a savings goal: optional, e.g. a daily "Save $5" — only when a natural next action is obvious from what the user said; each completion auto-logs its contribution, never a separate log_goal_entry for the same amount. For a HABIT goal: required, exactly the recurring check-in task ("Meditate 10 min", daily) — the streak counts that task and completing it IS the check-in. For an INDIRECT goal: optional, supporting activity only ("go for a run") — never carries a contribution, and completing it never logs a measurement (only log_goal_entry does that). For a MILESTONE goal: REQUIRED, and they are the tasks the USER named for the FIRST milestone — never ones you thought of. Milestone creation takes two answers: what the milestones are, then what they will actually DO to reach the first one. If you have the stages but the user has not said what gets them through the first one, do NOT call this tool yet: call no_action and the reply pass will ask. Never carries a contribution; a later stage\'s tasks are asked for at advance_goal_stage time, never here. Never invent a schedule, amount, or time the user didn\'t give (a plain "daily" gets no time field).',
        },
      },
      required: ['type', 'name'],
    },
  ...
  },
  {
    ...
    description:
      'MILESTONE goals only: request moving a milestone goal to its next stage — this does NOT move it immediately. It shows the user a card that lists the current stage\'s open tasks getting retired and the next stage\'s tasks, so they can confirm or cancel themselves; nothing changes until they tap Advance. Completing a linked task is NEVER by itself a reason to call this — a task finishing is not the same as the user declaring the stage done; only call this on the user\'s own clear statement.\n\nTIMING, and it has two cases. When the user declares the current stage done and a NEXT STAGE EXISTS, the next stage\'s tasks are the user\'s to give: if they haven\'t already said what they want to do for it, do NOT call this yet — call no_action instead (reason: need the next milestone\'s tasks) and the reply pass will ask them. Once they answer, call this with their answer in nextStageTasks. Note what is and isn\'t being asked: never ask "should I move you to the next stage?" — the tap on the card is that confirmation. The only question is what the NEXT stage\'s tasks are. When the user answers that in the same breath as declaring the stage done ("got the interview! now I need to do mock interviews and study leetcode"), you already have both — call this immediately. If this is the LAST stage there is no next stage and nothing to ask: call it right away, and it proposes completing the goal (no new tasks).\n\nUse the goal\'s ref from the goals list in context, never a guess.',
    input_schema: {
      type: 'object',
      properties: {
        goalRef: GOAL_REF_PROPERTY,
        ...
        nextStageTasks: {
          ...
          description:
            'The NEXT stage\'s to-dos (up to 5), exactly as the USER gave them — never a contribution amount, a milestone goal never logs a number from a task. Never invent these: if a next stage exists and the user hasn\'t said what they want to do for it, call no_action and ask, rather than calling this tool with tasks you made up. Omit entirely only when this advance completes the goal (no next stage, so nothing to ask for).',
        },
      },
      required: ['goalRef', 'nameHint'],
    },
