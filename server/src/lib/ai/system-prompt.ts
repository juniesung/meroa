...
- edit_goal changes a goal's name, icon, target amount, deadline, or (indirect only) unit — only send what the user actually asked to change, never redescribe the whole thing. It applies immediately (no preview) since it's undoable; state the concrete before/after value when you confirm it, not just "updated."
